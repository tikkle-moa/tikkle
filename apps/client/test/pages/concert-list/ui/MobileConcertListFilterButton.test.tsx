...
describe("MobileConcertListFilterButton", () => {
  it("필터가 없고 닫힌 상태면 기본 접근성 상태를 렌더링한다", () => {
    render(<MobileConcertListFilterButton isOpen={false} activeFilterCount={0} onClick={vi.fn()} />);
  ...
  it("열린 상태를 접근성 속성으로 렌더링한다", () => {
    render(<MobileConcertListFilterButton isOpen activeFilterCount={0} onClick={vi.fn()} />);

    expect(screen.getByRole("button", { name: "필터" })).toHaveAttribute("aria-expanded", "true");
  });

  it("선택된 필터 수를 숫자 배지와 접근성 이름으로 렌더링한다", () => {
    render(<MobileConcertListFilterButton isOpen={false} activeFilterCount={2} onClick={vi.fn()} />);
  ...
  it("클릭을 onClick에 위임한다", async () => {
    ...
    render(<MobileConcertListFilterButton isOpen={false} activeFilterCount={0} onClick={onClick} />);
