...
describe("useVenueSeatList", () => {
  it("스크롤 위치에 맞는 좌석을 제공한다", () => {
    const { result } = renderHook(() => useVenueSeatList({ venueSeats: seats, setSelectedSeatClientIds: vi.fn() }));
  ...
  it("일반 클릭과 Shift 추가 및 해제를 처리한다", () => {
    const setter = vi.fn();
    const { result } = renderHook(() => useVenueSeatList({ venueSeats: seats, setSelectedSeatClientIds: setter }));
